        ...
        <table *ngIf="!loading && !error && accounts.length > 0" class="table">
          ...
          <tbody>
            <tr *ngFor="let account of accounts" [class.row-overdue]="account.status === 'vermelho'">
              <td>{{ account.customers?.name || 'Cliente não encontrado' }}</td>
...
export class AccountsReceivableListComponent implements OnInit {
  ...
  async loadAccounts() {
    ...
    this.error = null;
    try {
      this.accounts = await this.accountsReceivableService.getAll();
    } catch (err: any) {
      this.error = err.message || 'Erro ao carregar contas a receber';
    } finally {
      this.loading = false;
    }
  }
  ...
  navigateToNew() {
    this.router.navigate(['/accounts-receivable/new']);
  }

  viewDetail(id: string) {
    this.router.navigate(['/accounts-receivable', id]);
  }

  editAccount(id: string) {
    this.router.navigate(['/accounts-receivable', id, 'edit']);
  }

  async markAsPaid(account: AccountReceivable) {
    if (!confirm(`Marcar conta de ${account.customers?.name} como paga?`)) {
      return;
    }
  ...
  async deleteAccount(id: string) {
    ...
    try {
      await this.accountsReceivableService.delete(id);
      await this.loadAccounts();
    } catch (err: any) {
  ...
  exportToExcel() {
    try {
      this.exportService.exportReceivablesToExcel(this.accounts);
  ...
  exportToPDF() {
    try {
      this.exportService.exportReceivablesToPDF(this.accounts);
