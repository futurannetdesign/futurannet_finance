    ...
    <div class="container">
      <div class="card">
        <div class="card-header">
          <h2>{{ isEditMode ? 'Editar Conta a Receber' : 'Nova Conta a Receber' }}</h2>
        </div>
        ...
        <form [formGroup]="accountForm" (ngSubmit)="onSubmit()" *ngIf="!loading">
          <div class="form-group">
            ...
            <select 
              ...
              [class.error]="accountForm.get('customer_id')?.invalid && accountForm.get('customer_id')?.touched">
              <option value="">Selecione um cliente</option>
              <option *ngFor="let customer of customers" [value]="customer.id">
                {{ customer.name }} - {{ customer.phone || 'Sem telefone' }}
              </option>
            </select>
            <div *ngIf="accountForm.get('customer_id')?.invalid && accountForm.get('customer_id')?.touched" class="error-message">
...
export class AccountsReceivableFormComponent implements OnInit {
  ...
  async loadAccount() {
    ...
    this.error = null;
    try {
      const account = await this.accountsReceivableService.getById(this.accountId);
      if (account) {
